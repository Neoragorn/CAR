...
public class MessageDiscussionBdd {
    ...
    public static void insertMessageIntoDiscussion(MessageDiscussion message, User user) {
        try {
            String req = "INSERT INTO Message (idGroup, idUser, message, heure) VALUES (1, ?, ?, ?)";

            PreparedStatement pss = conn.prepareStatement(req);
            pss.setInt(1, user.getIdUser());
            pss.setString(2, message.getMessage());
            pss.setDate(3, message.getTime());
            pss.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e);
        }
    ...
    public static ArrayList<MessageDiscussion> getPastMessageFromDiscussion() {
        try {
            ArrayList<MessageDiscussion> msgList = new ArrayList();
            String req = "SELECT msg.idMessage, u.pseudo, msg.message, msg.heure FROM Message msg "
                    + "JOIN User u on u.idUser = msg.idUser "
